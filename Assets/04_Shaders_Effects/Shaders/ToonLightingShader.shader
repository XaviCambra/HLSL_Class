...
Shader "Tecnocampus/ToonLightingShader"
{
    Properties
    {
        _MainTex("_MainTex", 2D) = "defaulttexture" {}
        _ToonTexture("_ToonTexture", 2D) = "defaulttexture" {}
        ...
        SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(VERTEX_OUT IN) : SV_Target
            {
                ...
                for (int i = 0; i < _LightsCount; i++)
                {
                    ...
                    Kd = tex2D(_ToonTexture, float2(Kd, 0.5)).x;
        ...
        Pass
        {
            ...
            {
                VERTEX_OUT o;
                float3 l_Vertex = v.vertex.xyz + v.normal.xyz * _OutlineWidth;
                o.vertex = mul(unity_ObjectToWorld, float4(l_Vertex, 1.0));
                o.vertex = mul(UNITY_MATRIX_V, o.vertex);
                o.vertex = mul(UNITY_MATRIX_P, o.vertex);
                return o;
            }
