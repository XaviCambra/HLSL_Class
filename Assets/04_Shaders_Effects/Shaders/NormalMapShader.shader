Shader "Tecnocampus/NormalMapShader"
{
    Properties
    {
        ...
        _BumpTexture("_BumpTexture", 2D) = "" {}
        _Bump("_Bump", Float) = 0.5
    }
        SubShader
    {
        ...
        Pass
        {
            ...
            VERTEX_OUT vert(VERTEX_IN v)
            {
                ...
                o.WorldPosition = normalize(mul((float3x3)unity_ObjectToWorld, v.normal));
                o.Tangent = normalize(mul((float3x3)unity_ObjectToWorld, v.Tangent));
                o.Binormal = normalize(mul((float3x3)unity_ObjectToWorld, normalize(cross(v.Tangent, v.normal))));
            ...
            float3 UnpackNormalMap(float4 BumpColor)
            {
                float3 l_Normal;
                l_Normal.xy = BumpColor.wy * 2 - 1;
                l_Normal.z = sqrt(1 - saturate(dot(l_Normal.xy, l_Normal.xy)));
                return l_Normal;
            }
            ...
            { 
                ...
                float4 l_NormalMapColor = tex2D(_BumpTexture, IN.uv);
                float3 l_Bump = _Bump * UnpackNormalMap(l_NormalMapColor);
                float3 Nn = IN.normal + (l_Bump.x * IN.Tangent + l_Bump.y * IN.Binormal);
                Nn = normalize(Nn);
